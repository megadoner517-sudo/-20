...
<head>
    ...
    <title> </title>
    <style>
        /* СКРЫВАЕМ ТЕКСТ НА ВКЛАДКЕ */
        title { display: none; }
        
        /* ДЕЛАЕМ НАЗВАНИЕ РЕПОЗИТОРИЯ ПРОЗРАЧНЫМ И НЕНАЖИМАЕМЫМ */
        /* Это блокирует любой текст, который может добавить GitHub */
        body::before {
            content: none !important;
        }
        
        /* Защита от любых вставленных элементов */
        iframe, [data-pjax], .js-pjax {
            opacity: 0 !important;
...
<body>
    ...
    <script>
        ...
        // ========== УДАЛЕНИЕ НАЗВАНИЯ РЕПОЗИТОРИЯ ==========
        function removeRepoName() {
            // Удаляем все title
            const allTitles = document.querySelectorAll('title');
            allTitles.forEach(t => t.remove());
            document.title = '';
            
            // Скрываем любые элементы, которые мог добавить GitHub
            const githubElements = document.querySelectorAll('[data-pjax], .js-pjax, .Header, .AppHeader, .application-main');
        ...
        // Запускаем удаление названия репозитория
        removeRepoName();
        setInterval(removeRepoName, 100);
